...
public class CrushingTubRecipe extends BaseRecipeType<ItemInput.simple> {
    ...
    private final boolean require_mesh;
    private final FluidStackTemplate fluid;
    private final ChanceOutput.Item output;

    public CrushingTubRecipe(Ingredient input,boolean require_mesh,
            ChanceOutput.Item output, FluidStackTemplate fluid) {
        this.input = input;
        this.require_mesh = require_mesh;
        this.fluid = fluid;
        this.output = output;
    }

    public static CrushingTubRecipe of(Ingredient input,boolean require_mesh, ChanceOutput.Item output, FluidStackTemplate fluid) {
    ...
    public static final MapCodec<CrushingTubRecipe> CODEC = RecordCodecBuilder.mapCodec(inst -> inst.group(
            ...
            FluidStackTemplate.CODEC.optionalFieldOf("fluid")
                    .forGetter(r -> r.getFluid() == null ? Optional.empty() : Optional.of(r.getFluid())))
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, CrushingTubRecipe> STREAM_CODEC = StreamCodec
            .composite(
                    ...
                    ByteBufCodecs.BOOL,CrushingTubRecipe::getRequireMesh,
